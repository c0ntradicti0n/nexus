...
#if defined(PLOG_ENABLE_GET_THIS) && defined(_MSC_VER) && _MSC_VER >= 1600 &&  \
    !defined(__INTELLISENSE__) && !defined(__INTEL_COMPILER) &&                \
    !defined(__llvm__) &&                                                      \
    !defined(__RESHARPER__) // >= Visual Studio 2010, skip IntelliSense, Intel
                            // Compiler, Clang Code Model and ReSharper
...
#ifdef _MSC_VER
#define IF_PLOG_(instanceId, severity)                                         \
    __pragma(warning(push)) __pragma(warning(disable : 4127)) if (true) { ; }  \
    else __pragma(warning(pop)) // conditional expression is constant
#else
#define IF_PLOG_(instanceId, severity)                                         \
    if (true) {                                                                \
        ;                                                                      \
...
#else
#define IF_PLOG_(instanceId, severity)                                         \
    if (!plog::get<instanceId>() ||                                            \
        !plog::get<instanceId>()->checkSeverity(severity)) {                   \
        ;                                                                      \
    } else
#endif
...
#define PLOG_(instanceId, severity)                                            \
    IF_PLOG_(instanceId, severity)                                             \
    (*plog::get<instanceId>()) +=                                              \
        plog::Record(severity, PLOG_GET_FUNC(), __LINE__, PLOG_GET_FILE(),     \
                     PLOG_GET_THIS(), instanceId)                              \
            .ref()
...
#define PLOG_IF_(instanceId, severity, condition)                              \
    if (!(condition)) {                                                        \
        ;                                                                      \
    } else                                                                     \
        PLOG_(instanceId, severity)
#define PLOG_IF(severity, condition)                                           \
    PLOG_IF_(PLOG_DEFAULT_INSTANCE_ID, severity, condition)
...
#define PLOG_VERBOSE_IF_(instanceId, condition)                                \
    PLOG_IF_(instanceId, plog::verbose, condition)
#define PLOG_DEBUG_IF_(instanceId, condition)                                  \
    PLOG_IF_(instanceId, plog::debug, condition)
#define PLOG_INFO_IF_(instanceId, condition)                                   \
    PLOG_IF_(instanceId, plog::info, condition)
#define PLOG_WARNING_IF_(instanceId, condition)                                \
    PLOG_IF_(instanceId, plog::warning, condition)
#define PLOG_ERROR_IF_(instanceId, condition)                                  \
    PLOG_IF_(instanceId, plog::error, condition)
#define PLOG_FATAL_IF_(instanceId, condition)                                  \
    PLOG_IF_(instanceId, plog::fatal, condition)
#define PLOG_NONE_IF_(instanceId, condition)                                   \
    PLOG_IF_(instanceId, plog::none, condition)
...
// Old macro names for downward compatibility. To bypass including these macro
// names, add #define PLOG_OMIT_LOG_DEFINES before #include <plog/Log.h>
#ifndef PLOG_OMIT_LOG_DEFINES

//////////////////////////////////////////////////////////////////////////
// Main logging macros - can be changed later to point at macros for a different
// logging package

#define LOG_(instanceId, severity)                                             \
    IF_PLOG_(instanceId, severity)                                             \
    (*plog::get<instanceId>()) +=                                              \
        plog::Record(severity, PLOG_GET_FUNC(), __LINE__, PLOG_GET_FILE(),     \
                     PLOG_GET_THIS(), instanceId)                              \
...
#define LOG_IF_(instanceId, severity, condition)                               \
    if (!(condition)) {                                                        \
        ;                                                                      \
    } else                                                                     \
        PLOG_(instanceId, severity)
...
#define LOG_VERBOSE_IF_(instanceId, condition)                                 \
    PLOG_IF_(instanceId, plog::verbose, condition)
#define LOG_DEBUG_IF_(instanceId, condition)                                   \
    PLOG_IF_(instanceId, plog::debug, condition)
#define LOG_INFO_IF_(instanceId, condition)                                    \
    PLOG_IF_(instanceId, plog::info, condition)
#define LOG_WARNING_IF_(instanceId, condition)                                 \
    PLOG_IF_(instanceId, plog::warning, condition)
#define LOG_ERROR_IF_(instanceId, condition)                                   \
    PLOG_IF_(instanceId, plog::error, condition)
#define LOG_FATAL_IF_(instanceId, condition)                                   \
    PLOG_IF_(instanceId, plog::fatal, condition)
#define LOG_NONE_IF_(instanceId, condition)                                    \
    PLOG_IF_(instanceId, plog::none, condition)
