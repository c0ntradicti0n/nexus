...
namespace detail {
namespace meta {
template <class T> inline T &declval() {
...
template <class From, class To> struct isConvertible {
    ...
    template <class U>
    static typename enableIf<!!(sizeof(static_cast<To>(meta::declval<U>())) +
                                sizeof(U *)),
                             Yes>::type
    test(int);
...
#if defined(_MSC_VER) &&                                                       \
    _MSC_VER <                                                                 \
        1700 // MSVC 2010 doesn't understand `typename T::const_iterator`
                                          meta::declval<U>().begin()) +
                                      sizeof(meta::declval<U>().end()
#else
...
inline void operator<<(util::nostringstream &stream,
...
#ifdef _WIN32
namespace meta {
template <class T, class Stream> inline No operator<<(Stream &, const T &);

template <class T, class Stream> struct isStreamable {
    enum {
        value = sizeof(operator<<(meta::declval<Stream>(),
                                  meta::declval<const T>())) != sizeof(No)
    };
};
...
template <class Stream, size_t N> struct isStreamable<wchar_t[N], Stream> {
    enum { value = false };
};
...
class Record {
  public:
    Record(Severity severity, const char *func, size_t line, const char *file,
           const void *object, int instanceId)
        : m_severity(severity), m_tid(util::gettid()), m_object(object),
          m_line(line), m_func(func), m_file(file), m_instanceId(instanceId) {
        util::ftime(&m_time);
    }
...
#ifdef QSTRINGVIEW_H
    Record &operator<<(QStringView data) { return *this << data.toString(); }
#endif
#endif
    ...
    virtual const util::Time &getTime() const { return m_time; }

    virtual Severity getSeverity() const { return m_severity; }

    virtual unsigned int getTid() const { return m_tid; }
    ...
    virtual const char *getFile() const { return m_file; }

    virtual ~Record() // virtual destructor to satisfy -Wnon-virtual-dtor
                      // warning
    {}

    virtual int getInstanceId() const { return m_instanceId; }
