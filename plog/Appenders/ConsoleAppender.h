...
#pragma warning(suppress : 26812) //  Prefer 'enum class' over 'enum'
    ...
    ConsoleAppender(OutputStream outStream = streamStdOut)
        : m_isatty(
              !!_isatty(_fileno(outStream == streamStdOut ? stdout : stderr))),
          m_outputStream(outStream == streamStdOut ? std::cout : std::cerr),
          m_outputHandle() {
        if (m_isatty) {
            m_outputHandle = GetStdHandle(outStream == streamStdOut
                                              ? stdHandle::kOutput
                                              : stdHandle::kErrorOutput);
        }
    }
#else
    ConsoleAppender(OutputStream outStream = streamStdOut)
        : m_isatty(
              !!isatty(fileno(outStream == streamStdOut ? stdout : stderr))),
          m_outputStream(outStream == streamStdOut ? std::cout : std::cerr) {}
#endif
...
#ifdef _WIN32
        if (m_isatty) {
            WriteConsoleW(m_outputHandle, str.c_str(),
                          static_cast<DWORD>(str.size()), NULL, NULL);
        } else {
            m_outputStream << util::toNarrow(str, codePage::kActive)
                           << std::flush;
        }
#else
