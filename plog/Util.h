...
inline std::wstring toWide(const char *str) {
    ...
    if (!wstr.empty()) {
        int wlen = MultiByteToWideChar(codePage::kActive, 0, str,
                                       static_cast<int>(len), &wstr[0],
...
inline std::string toNarrow(const std::wstring &wstr, long page) {
    ...
    if (!str.empty()) {
        WideCharToMultiByte(page, 0, wstr.c_str(),
                            static_cast<int>(wstr.size()), &str[0], len, 0, 0);
    }
