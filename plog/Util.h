...
inline int vaswprintf(wchar_t **strp, const wchar_t *format, va_list ap) {
  ...
  wchar_t *str =
      static_cast<wchar_t *>(malloc(bufferCharCount * sizeof(wchar_t)));
  if (!str) {
    return -1;
  }
...
inline std::wstring toWide(const char *str) {
  ...
  if (!wstr.empty()) {
    int wlen =
        MultiByteToWideChar(codePage::kActive, 0, str, static_cast<int>(len),
...
inline std::string toNarrow(const std::wstring &wstr, long page) {
  int len = WideCharToMultiByte(page, 0, wstr.c_str(),
                                static_cast<int>(wstr.size()), 0, 0, 0, 0);
  std::string str(len, 0);

  if (!str.empty()) {
    WideCharToMultiByte(page, 0, wstr.c_str(), static_cast<int>(wstr.size()),
                        &str[0], len, 0, 0);
  }
...
inline void splitFileName(const nchar *fileName, nstring &fileNameNoExt,
                          nstring &fileExt) {
...
public:
  ...
  File(const nchar *fileName) : m_file(-1) { open(fileName); }
  ...
  size_t open(const nchar *fileName) {
    ...
    m_file =
        ::_wsopen(fileName, _O_CREAT | _O_WRONLY | _O_BINARY | _O_NOINHERIT,
                  SH_DENYWR, _S_IREAD | _S_IWRITE);
#elif defined(_WIN32)
    ::_wsopen_s(&m_file, fileName,
                _O_CREAT | _O_WRONLY | _O_BINARY | _O_NOINHERIT, _SH_DENYWR,
                _S_IREAD | _S_IWRITE);
#elif defined(O_CLOEXEC)
    m_file = ::open(fileName, O_CREAT | O_WRONLY | O_CLOEXEC,
...
public:
  Mutex() {
    ...
    rtems_semaphore_create(
        0, 1, RTEMS_PRIORITY | RTEMS_BINARY_SEMAPHORE | RTEMS_INHERIT_PRIORITY,
        1, &m_sync);
