...
public:
  HexDump(const void *ptr, size_t size)
      : m_ptr(static_cast<const unsigned char *>(ptr)), m_size(size),
        m_group(8), m_digitSeparator(" "), m_groupSeparator("  ") {}
  ...
  friend util::nostringstream &operator<<(util::nostringstream &stream,
                                          const HexDump &);
...
inline util::nostringstream &operator<<(util::nostringstream &stream,
...
inline HexDump hexdump(const void *ptr, size_t size) {
  return HexDump(ptr, size);
}

template <class Container> inline HexDump hexdump(const Container &container) {
  return HexDump(container.data(),
                 container.size() * sizeof(*container.data()));
}

template <class T, size_t N> inline HexDump hexdump(const T (&arr)[N]) {
  return HexDump(arr, N * sizeof(*arr));
}
} // namespace plog
